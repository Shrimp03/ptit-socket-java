...
public class ChatPanel extends JPanel implements Runnable {
	...
	private JScrollPane scrollPane_mesg;
	...
	// run method is to adding a thread
	@Override
	public void run() {
		while (true) {
			try {
	...
	public ChatPanel(Socket s, String sender, String receiver) {
		...
		try {
			...
			groupLayout.setVerticalGroup(groupLayout.createParallelGroup(Alignment.LEADING).addGroup(groupLayout
					...
					.addComponent(getLblHistory(), GroupLayout.DEFAULT_SIZE, 27, Short.MAX_VALUE)
	...
	public JScrollPane getTextArea() {
		if (textArea == null) {
			...
			scrollPane_mesg = new JScrollPane(textArea);
			scrollPane_mesg.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
			scrollPane_mesg.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		}
		return scrollPane_mesg;
	}
	...
	public JScrollPane getTextArea_1() {
		if (textArea_1 == null) {
			textArea_1 = new JTextArea();
			textArea_1.setFont(new Font("Arial", Font.PLAIN, 30));
			scrollBar = new JScrollPane(textArea_1);
